...
from util import RSSI_MAX, RSSI_MIN, X_MAX, Y_MAX, Z_MAX

app = Flask(__name__)

name = 'Direct location prediction-06-08--14-46-58'
model = pickle.load(open(f"sklearn_models/multilayer/{name}.pkl", 'rb'))
...
def predict():
    ...
    # for item in data:
    #     row = mac_to_ap_df[mac_to_ap_df["Base Radio MAC Address"] == item['MAC']]
    #     if not row.empty:
    #         dictio[row["AP Name"].values[0]] = item['signalStrength']
